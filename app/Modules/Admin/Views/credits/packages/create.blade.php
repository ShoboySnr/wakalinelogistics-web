...
<div class="px-4 sm:px-6 lg:px-0">
    <div class="mb-6">
        <div class="flex items-center gap-3 mb-2">
            ...
            <h1 class="text-2xl font-bold text-gray-900">Create Credit Package</h1>
        </div>
    </div>

    <form action="{{ route('admin.credits.packages.store') }}" method="POST" class="bg-white rounded-lg shadow p-6">
        @csrf
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="name" value="{{ old('name') }}" required
                    ...
                    placeholder="e.g., 50 Deliveries">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Price (₦) *</label>
                <input type="number" name="price" value="{{ old('price') }}" required step="any" min="0"
                    class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-pink-500 focus:border-pink-500">
            </div>
            <div>
            ...
            <div>
                ...
                <input type="number" name="bonus_credits" value="{{ old('bonus_credits', 0) }}" min="0"
                    class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-pink-500 focus:border-pink-500">
            </div>
            <div>
        ...
        <div class="mt-6">
            ...
            <textarea name="description" rows="3"
                class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-pink-500 focus:border-pink-500">{{ old('description') }}</textarea>
        </div>
        ...
        <div class="mt-8 flex gap-3">
            <button type="submit" class="px-6 py-2 text-white rounded-md brand-accent-bg brand-accent-hover">Create Package</button>
