    ...
    <div class="bg-white shadow rounded-lg overflow-hidden">
        ...
        <form action="{{ route('admin.orders.update', $order->id) }}" method="POST" enctype="multipart/form-data" class="px-6 py-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2 bg-pink-50 border-2 rounded-lg p-4 mb-4" style="border-color: #C1666B;">
                    <h3 class="text-sm font-semibold mb-3 flex items-center brand-accent-text">
                    ...
                    <div>
                        ...
                        <select name="client_id" id="client_id"
                            ...
                            <option value="">-- Select a client to auto-fill pickup details --</option>
                            @foreach(\App\Modules\Admin\Models\Client::where('is_active', true)->orderBy('name')->get() as $client)
                                <option value="{{ $client->id }}" {{ old('client_id', $order->client_id) == $client->id ? 'selected' : '' }}>
                                    {{ $client->name }}{{ $client->company_name ? ' (' . $client->company_name . ')' : '' }} - {{ $client->phone }}
                                </option>
                            @endforeach
                        </select>
                        <p class="mt-1 text-xs text-gray-600">Selecting a client will automatically fill in the pickup information below</p>
                    </div>
                </div>
                ...
                <div class="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6 mt-2">
                    <div>
                        ...
                        <select name="payment_method" id="payment_method"
                            ...
                            <option value="cash" {{ old('payment_method', $order->payment_method) == 'cash' ? 'selected' : '' }}>Cash (Pay on Delivery)</option>
                            <option value="credits" {{ old('payment_method', $order->payment_method) == 'credits' ? 'selected' : '' }}>Credits</option>
                            <option value="card" {{ old('payment_method', $order->payment_method) == 'card' ? 'selected' : '' }}>Card</option>
                            <option value="subscription" {{ old('payment_method', $order->payment_method) == 'subscription' ? 'selected' : '' }}>Subscription</option>
                        </select>
                    </div>
                    <div id="amount_received_wrapper" class="{{ old('payment_method', $order->payment_method) == 'cash' ? '' : 'hidden' }}">
                ...
                <!-- Remittance -->
                <div id="remittance_wrapper" class="md:col-span-2 {{ old('payment_method', $order->payment_method) == 'cash' && $order->amount_received !== null ? '' : 'hidden' }}">
                    <div class="rounded-lg p-4" style="background-color: #fdf1f1; border: 1px solid #e8a0a4;">
                        <label class="flex items-center gap-3 cursor-pointer">
                            <input type="checkbox" name="mark_remitted" id="mark_remitted" value="1"
                                   ...
                                   {{ old('mark_remitted', $order->remitted_at ? '1' : '') == '1' ? 'checked' : '' }}>
                            <span class="text-sm font-medium text-gray-700">Mark as Remitted</span>
                        </label>
                        @if($order->remitted_at)
                        <p class="mt-1 ml-7 text-xs text-gray-500">Remitted on {{ $order->remitted_at->format('M d, Y h:i A') }}</p>
                        @endif
                    </div>
                </div>
                    ...
                    <div class="rounded-lg p-4 border border-red-200 bg-red-50">
                        <label class="flex items-center gap-3 cursor-pointer">
                            <input type="checkbox" name="is_failed_delivery" id="is_failed_delivery" value="1"
                                   ...
                                   {{ old('is_failed_delivery', $order->is_failed_delivery) ? 'checked' : '' }}>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.toggleAmountReceived = function(value) {
