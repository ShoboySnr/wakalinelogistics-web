...
class ValidateApiToken
{
    public function handle(Request $request, Closure $next, ?string $expected = null): Response
    {
        ...
        $botToken = config('services.whatsapp_bot_api.token');
        ...
        if (!$token || !in_array($token, array_filter($accepted), true)) {
            if ($expected === 'frontend') {
                ...
                $errorMessage = 'Unauthorized. Valid WhatsApp Bot API key required.';
                $errorCode = 'INVALID_BOT_API_KEY';
            } else {
